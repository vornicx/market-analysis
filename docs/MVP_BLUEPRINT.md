...
| `feature_windows` | `selection_id FK`, `bookmaker_key null` (null = consensus), `window_key text` (`poll`,`1h`,`6h`,`24h`; named `window_key` because `window` is reserved in Postgres), `dp numeric`, `z numeric`, `pctile numeric`, `computed_at` | Upsert on `(selection_id, bookmaker_key, window_key)`. Retention 14d. |
